"""
DPM-Solver / DPM-Solver++ (Lu et al., 2022): fast ODE samplers for diffusion
models. The core (NoiseScheduleVP, model_wrapper, DPM_Solver, interpolate_fn,
expand_dims) is vendored verbatim from the UniPC repo's Stable-Diffusion
integration, which is the reference implementation by Cheng Lu:
  https://github.com/wl-zhao/UniPC  (example/stable-diffusion/.../dpm_solver)
  https://github.com/LuChengTHU/dpm-solver
Only the DPMSolverSampler adapter at the bottom of this file is LDCast-specific;
it mirrors PLMSSampler.sample so it is a drop-in replacement.
"""
import torch
...
class DPMSolverSampler:
    """DPM-Solver++ (multistep) drop-in replacement for PLMSSampler.

    Same call contract as PLMSSampler.sample used by ldcast.forecast.Forecast
    and scripts/eval_genforecast.py:
        sample(S, batch_size, shape, conditioning, ..., progbar=...)
            -> (samples, intermediates)
    LDCast uses eps-prediction on a linear discrete schedule
    (model.alphas_cumprod), no latent scale_factor and no classifier-free
    guidance, which is exactly the configuration this solver expects.
    """

    def __init__(self, model, order=2, **kwargs):
        self.model = model
        self.order = order
